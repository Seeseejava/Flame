...
namespace Flame
{
	...
	void NativeOpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		FLAME_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders for now");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				FLAME_CORE_ASSERT(false, "Shader compilation failure!")
					break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			FLAME_CORE_ERROR("{0}", infoLog.data());
			FLAME_CORE_ASSERT(false, "NativeOpenGLShader link failure!")
				return;
		}
