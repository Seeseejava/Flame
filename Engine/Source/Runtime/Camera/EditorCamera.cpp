...
namespace Flame {
	...
	{
		if (Input::IsKeyPressed(FLAME_KEY_LEFT_ALT))
		{
			const glm::vec2& mouse{ Input::GetMouseX(), Input::GetMouseY() };
			glm::vec2 delta = (mouse - m_InitialMousePosition) * 0.003f;
			m_InitialMousePosition = mouse;

			if (Input::IsMouseButtonPressed(FLAME_MOUSE_BUTTON_MIDDLE))
				MousePan(delta);
			else if (Input::IsMouseButtonPressed(FLAME_MOUSE_BUTTON_LEFT))
				MouseRotate(delta);
	...
	void EditorCamera::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseScrolledEvent>(FLAME_BIND_EVENT_FN(EditorCamera::OnMouseScroll));
	}
	...
	{
		return glm::quat(glm::vec3(-m_Pitch, -m_Yaw, 0.0f));  // ËÄÔªÊý
	}
